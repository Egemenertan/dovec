...
export default function NewBlogPage() {
      ...
      input.onchange = async () => {
        ...
        if (file) {
          try {
            ...
            const storageRef = ref(storage, `blog/content/${timestamp}_${file.name}`)
            
            // Yükleme işlemi başladı
            const uploadResult = await uploadBytes(storageRef, file)
            console.log('Görsel yüklendi:', uploadResult)
            
            // URL'i al
            const imageUrl = await getDownloadURL(uploadResult.ref)
            console.log('Görsel URL:', imageUrl)

            // Editöre görseli ekle
  ...
  const updateParagraph = (index: number, field: keyof Paragraph, value: string | File) => {
    ...
    if (field === 'imageFile' && value instanceof File) {
      // Görsel dosyası için önizleme URL'i oluştur
      const reader = new FileReader()
      reader.onloadend = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Kapak resmini yükle
      const timestamp = Date.now()
      const storageRef = ref(storage, `blog/covers/${timestamp}_${selectedImage.name}`)
      const uploadResult = await uploadBytes(storageRef, selectedImage)
      const imageUrl = await getDownloadURL(uploadResult.ref)

      // Paragrafları ve görsellerini yükle
      let additionalContent = ''
      for (const para of paragraphs) {
        let paraImageUrl = ''
        if (para.imageFile) {
          const paraImageRef = ref(storage, `blog/paragraphs/${timestamp}_${para.imageFile.name}`)
        ...
        if (para.title || para.content || paraImageUrl) {
          additionalContent += `
            ...
            ${paraImageUrl ? `<img src="${paraImageUrl}" alt="${para.title || 'Blog görseli'}" class="rounded-xl shadow-lg my-8 w-full" loading="lazy">` : ''}
          `
        }
      }

      // HTML içeriğini temizle ve başlık stillerini düzelt
      ...
      // Blog verisini Firestore'a ekle
